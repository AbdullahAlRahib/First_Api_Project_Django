...
from First_App.models import Musicians,Album
from First_App import forms
from django.db.models import Avg

# Create your views here.
def index(request):
    musician_list = Musicians.objects.order_by('first_name')
    diction={
        'title': 'Home Page ',
...
def album_list(request, artist_id):
    artist_info =Musicians.objects.get(pk=artist_id)
...
def musicians_form(request):
    ...
    return render(request,'first_app/musicians_form.html',context=diction)
...
def delete_musicians(request, artist_id):
    artist =Musicians.objects.get(pk=artist_id).delete(0)
    diction ={
        'delete_success': 'Musicians Deleted Successfully!',
    }
    return render(request,'first_app/delete.html',context=diction)